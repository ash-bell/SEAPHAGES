...
def runGeneCallers(infile):
	...
	print(f"{bcolours.HEADER}Summoning the Dwarves {bcolours.ENDC}")
	runProdigal(infile)
	print(f"{bcolours.OKBLUE}Dwarf One has arrived {bcolours.ENDC}")
	runGlimmer(infile)
	print(f"{bcolours.OKBLUE}Dwarf Two has arrived {bcolours.ENDC}")
	runMGA(infile)
	print(f"{bcolours.OKBLUE}Dwarf Three has arrived {bcolours.ENDC}")
	runGeneMarkS(infile)
	print(f"{bcolours.OKBLUE}Dwarf Four has arrived {bcolours.ENDC}")
	runGeneMark(infile)
	print(f"{bcolours.OKBLUE}Dwarf Five has arrived {bcolours.ENDC}")
	runGeneMarkS2(infile)
	print(f"{bcolours.OKBLUE}Dwarf Six has arrived {bcolours.ENDC}")
...
def renameGeneCalls():
	...
	'''
print(f"{bcolours.OKGREEN}Renaming the Dwarves to something more sensible {bcolours.ENDC}")
...
def getGeneStats():
	...
	'''
	print(f"{bcolours.WARNING}Measuring Dwarves {bcolours.ENDC}")
	Stats = f"""# Prodigal;
		grep ^[^#] Prodigal.lst | cut -f4,5,6,7 > Prodigal.trm.lst;
...
def runCD_hit():
	...
	'''
	print(f"{bcolours.FAIL}Checking which dwarves are related{bcolours.ENDC}")
...
def combineGeneClusters():
	...
	'''
	print(f"{bcolours.BOLD}Combining related dwarves together{bcolours.ENDC}")
	#import DFs
	...
	Prodigal = pd.read_csv('Prodigal_genecalls.tsv', sep = "\t", names = ["id", "Sstart", "Send", "Gene_score","Direction"], index_col=False)
	df = pd.read_csv("Gene_clstr.txt", sep = "\t")

	# normalise the genescores
	print(f"{bcolours.UNDERLINE}Leveling the playing field{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Calculating Dwarf Height{bcolours.ENDC}")
	...
	print(f"{bcolours.OKBLUE}Penalising Short Dwarves{bcolours.ENDC}")
	...
	print(f"{bcolours.WARNING}Scoring the Dwarves{bcolours.ENDC}")
...
def GeneOverlap(result_df):
	...
	print(f"{bcolours.OKGREEN}Checking if Top Notch Dwarves overlap with the rest{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Checking if Not Bad Dwarves overlap with the rest{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Checking if Alright Dwarves overlap with the rest{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Checking if Meh Dwarves overlap with the rest{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Checking if Bad Dwarves overlap with the rest{bcolours.ENDC}")
	...
	print(f"{bcolours.OKGREEN}Checking if Dwarves runts overlap with themselves{bcolours.ENDC}")
	...
	print(f"{bcolours.OKBLUE}Scoring overlapping Dwarves{bcolours.ENDC}")
	...
	print(f"{bcolours.HEADER}Only keeping Dwarves over the age of 3{bcolours.ENDC}")
	final_df = results_df[results_df["total_score"] >= 3]
	
	# Sort the dataframe by gene start
	print(f"{bcolours.OKGREEN}Sorting the Dwarves by Height{bcolours.ENDC}")
	sorted_df = final_df.sort_values(by=["Sstart"]).reset_index(drop=True)
	
	# Save the sorted_df to file
	print(f"{bcolours.OKGREEN}Dwarf Statistics can be found here: FINAL_DF.tsv{bcolours.ENDC}")
	sorted_df.to_csv("FINAL_DF.tsv", sep = "\t", index = None, header = True)
...
def main(args):
	...
	runCD_hit()
	combineGeneClusters()
	GeneOverlap(combineGeneClusters())
